...
def get_grade_by_github_title(github, title):
    ...
    SELECT Students.first_name, Students.last_name,
           Grades.grade, Grades.project_title
           FROM Students JOIN Grades
            ON (students.github=grades.student_github)
            WHERE grades.project_title = :title
            AND Students.github = :github
    """
    db_cursor = db.session.execute(QUERY, {'github': github, 'title': title})
    row = db_cursor.fetchone()
    print "%s %s scored %d on %s" % (row[0], row[1], row[2], row[3])
...
def assign_grade(github, title, grade):
    ...
    db.session.commit()
    print "Sucessfully added grade: %d on project: %s for student: %s" % (grade, title, github)
